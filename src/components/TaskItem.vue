...
<template>
  <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 space-y-4 border border-slate-200 dark:border-slate-700
              transition-all duration-300 ease-in-out hover:shadow-xl hover:-translate-y-1
              flex flex-col h-full"> 

    <!-- Title and Status -->
    <div class="flex justify-between items-start mb-2">
      <h3 class="font-bold text-xl text-slate-900 dark:text-slate-100 flex-1 pr-4"
    ...
    <!-- Description -->
    <p v-if="task.description" class="text-slate-600 dark:text-slate-400 text-sm leading-relaxed flex-grow overflow-hidden">
      {{ task.description }}
    </p>
    <p v-else class="text-slate-400 dark:text-slate-600 text-sm flex-grow">No description provided.</p>

    <!-- Due Date (Optional) -->
    <div v-if="task.dueDate" class="flex items-center text-slate-500 dark:text-slate-400 text-xs mt-2">
    ...
    <!-- Action Buttons -->
    <div class="flex justify-end items-center space-x-3 pt-4 border-t border-slate-200 dark:border-slate-700 mt-auto">
      <button
